...
struct ZyklusAnalyseView: View {
    ...
    // All values precomputed at top level — no let inside nested ViewBuilder closures.
    @ViewBuilder
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 0) {
                VStack(spacing: 4) {
                    ...
                    Text("Vorhersage Zyklus").font(.caption2).foregroundStyle(.secondary)
                    if zyklusDiff != 0 {
                ...
                VStack(spacing: 4) {
                    ...
                    Text("Vorhersage Periode").font(.caption2).foregroundStyle(.secondary)
                    if periodDiff != 0 {
                ...
                if let offset = ovOffset, let diff = ovDiff {
                    Divider().frame(height: 44)
                    VStack(spacing: 4) {
                        Text("Tag \(offset)").font(.subheadline.bold()).foregroundStyle(.teal)
                        Text("Eisprung (gelernt)").font(.caption2).foregroundStyle(.secondary)
                        if diff != 0 {
    ...
    private var genauigkeitsKarte: some View {
        // Precompute everything before any if — avoids let-inside-if-branch in @ViewBuilder
        ...
        let chartHoehe      = CGFloat(max(fehler.count, 1)) * 18.0
        ...
        if fehler.count >= 2 {
            karte {
                    ...
                    VStack(alignment: .trailing, spacing: 2) {
                        ...
                        HStack(spacing: 3) {
                            Image(systemName: verbessert ? "arrow.down.circle.fill" :
                                ...
                                .font(.caption2)
                            Text(verbessert ? "Wird besser" :
                                 verschlechtert ? "Schwankend" : "Stabil")
                                .font(.caption2).foregroundStyle(.secondary)
                        }
                    }
                }
                ...
                .frame(height: max(chartHoehe, 100))
